...
--
-- Servidor: localhost
-- Tempo de Geração: 15/05/2015 às 16h45min
...
INSERT INTO `SYSMODULO` (`ID`, `NOME`, `SLUG`) VALUES
(1, 'Funcionário', 'funcionario'),
(2, 'Medicamento', 'medicamento'),
(3, 'Cliente', 'cliente');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_acesso` AS select `SP`.`ID` AS `ID_PERFIL`,`SM`.`ID` AS `ID_MODULO`,`SM`.`NOME` AS `MODULO`,`SF`.`ID` AS `ID_FUNCAO`,`SF`.`NOME` AS `FUNCAO` from (((`SYSPERFIL` `SP` join `SYSPERFIL_SYSFUNCAO` `SPS` on((`SPS`.`ID_PERFIL` = `SP`.`ID`))) join `SYSMODULO` `SM` on((`SM`.`ID` = `SPS`.`ID_MODULO`))) join `SYSFUNCAO` `SF` on((`SF`.`ID` = `SPS`.`ID_FUNCAO`)));
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_login` AS select `SU`.`ID` AS `ID`,`SU`.`LOGIN` AS `LOGIN`,`SU`.`SENHA` AS `SENHA`,`SP`.`NOME` AS `PERFIL`,`SU`.`FK_SYSPERFIL` AS `ID_PERFIL`,`F`.`NOME` AS `NOME`,`F`.`EMAIL` AS `EMAIL`,`F`.`STATUS` AS `STATUS`,`F`.`FOTO` AS `FOTO` from (((`SYSUSUARIO` `SU` join `SYSPERFIL` `SP` on((`SP`.`ID` = `SU`.`FK_SYSPERFIL`))) join `SYSPERFIL_SYSFUNCAO` `SPS` on((`SPS`.`ID_PERFIL` = `SU`.`FK_SYSPERFIL`))) join `FUNCIONARIO` `F` on((`F`.`ID` = `SU`.`ID_FUNCIONARIO`))) group by `SU`.`ID`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_moduloMenu` AS select distinct `SM`.`ID` AS `ID`,`SM`.`NOME` AS `NOME`,`SM`.`SLUG` AS `SLUG`,`SPS`.`ID_PERFIL` AS `ID_PERFIL` from ((`SYSMODULO` `SM` join `SYSPERFIL_SYSFUNCAO` `SPS` on((`SPS`.`ID_MODULO` = `SM`.`ID`))) join `SYSPERFIL` `SP` on((`SPS`.`ID_PERFIL` = `SP`.`ID`)));
...
ALTER TABLE `PRESSAO_ARTERIAL_MENSAL`
  ADD CONSTRAINT `PRESSAO_ARTERIAL_MENSAL_ibfk_1` FOREIGN KEY (`ID_CLIENTE`) REFERENCES `CLIENTES` (`ID`);
