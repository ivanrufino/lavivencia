...
--
-- Servidor: localhost
-- Tempo de Geração: 29/05/2015 às 17h01min
...
(1, 'Ivan Rufino', NULL, NULL, NULL, '1983-02-17', '4', '1', 'N', 1, 'N', 'N', 1, '', '0000-00-00', 1, '', '', '', '', '', '', 'N', '0000-00-00 00:00:00', ''),
(2, 'Outro ivan', NULL, NULL, NULL, '1980-01-01', '4', '2', 'N', 1, 'N', 'N', 2, '', '0000-00-00', 1, '', '', '', '', '', '', 'S', '0000-00-00 00:00:00', ''),
(3, 'rufino Ivan', NULL, ' ', NULL, '2015-05-03', '', '', '0', 1, '0', '0', 14, '123123', NULL, 1, '', '', ' ', '', '', ' ', 'S', '2015-05-29 19:53:27', '');
...
(14, 'Ivan contato', '', '', '', '', '', 1, '', '', '', '', '', '', '1983-05-05 00:00:00');
...
--
-- Estrutura stand-in para visualizar `v_cliente`
--
...
,`QUARTO` varchar(10)
,`CAMA` varchar(10)
,`TEMPORARIO` varchar(1)
,`FK_PLANO_SAUDE` int(11)
,`COBRE_INTERNACAO` varchar(1)
,`COBRE_REMOCAO` varchar(1)
,`FK_CONTATO` int(11)
...
--
-- Estrutura para visualizar `v_cliente`
--
DROP TABLE IF EXISTS `v_cliente`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_cliente` AS select `CLIENTE`.`ID` AS `ID`,`CLIENTE`.`NOME` AS `NOME`,`CLIENTE`.`FOTO` AS `FOTO`,`CLIENTE`.`OBSERVACAO` AS `OBSERVACAO`,`CLIENTE`.`DT_ALTERACAO` AS `DT_ALTERACAO`,`CLIENTE`.`DT_NASCIMENTO` AS `DT_NASCIMENTO`,`CLIENTE`.`QUARTO` AS `QUARTO`,`CLIENTE`.`CAMA` AS `CAMA`,`CLIENTE`.`TEMPORARIO` AS `TEMPORARIO`,`CLIENTE`.`FK_PLANO_SAUDE` AS `FK_PLANO_SAUDE`,`CLIENTE`.`COBRE_INTERNACAO` AS `COBRE_INTERNACAO`,`CLIENTE`.`COBRE_REMOCAO` AS `COBRE_REMOCAO`,`CLIENTE`.`FK_CONTATO` AS `FK_CONTATO`,`CLIENTE`.`INSCRICAO` AS `INSCRICAO`,`CLIENTE`.`DT_SAIDA` AS `DT_SAIDA`,`CLIENTE`.`FK_DIETA` AS `FK_DIETA`,`CLIENTE`.`IDENTIDADE` AS `IDENTIDADE`,`CLIENTE`.`ORGAO` AS `ORGAO`,`CLIENTE`.`DIAGNOSTICO` AS `DIAGNOSTICO`,`CLIENTE`.`CPF` AS `CPF`,`CLIENTE`.`EST_CIVIL` AS `EST_CIVIL`,`CLIENTE`.`MOTIVO_SAIDA` AS `MOTIVO_SAIDA`,`CLIENTE`.`ATIVO` AS `ATIVO`,`CLIENTE`.`DT_CADASTRO` AS `DT_CADASTRO`,`CLIENTE`.`ORIGEM` AS `ORIGEM`,(case `CLIENTE`.`ATIVO` when 'S' then (_utf8'ATIVO' collate utf8_general_ci) when 'N' then (_utf8'INATIVO' collate utf8_general_ci) end) AS `STATUS_LABEL` from `CLIENTE` where 1;
